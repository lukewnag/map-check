# VERSION 2: display will now compare each map to the maps with the hehighest regression score
# and the most minority opportunity districts instead of just whatever the final partition was
...
# or: 53.412, nat lean = 5.527 --> 47.885
    ...
    def partisan_coloring(partition):
        ...
        for district in partition.parts:
            ...
            dem_proportions[district] = num_dem_votes/(num_dem_votes+num_gop_votes)
    ...
    figMap, axMap = plt.subplots(4, 3, figsize = (14, 7), height_ratios = [6,6,6,1])
    figMap.subplots_adjust(top=0.8, bottom=0.1)
    figMap.suptitle(f"Comparing the {STATE} Districts of {YEAR} to Ensemble-Drawn Plans Maximizing Regression and Minority Representation\n ")
    
    row = 0
    for partition in [initial_partition, regression_partition, minority_partition]:
    ...
    for col in range(3):
        ...
        axMap[3, col].set_title(colorbar_desc[col])
        colorbar.ColorbarBase(ax=axMap[3,col], cmap=bar, orientation = 'horizontal')
