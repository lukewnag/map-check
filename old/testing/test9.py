...
nodeDict = {}
for node in graph:
    totpop, white, black = graph.nodes[node][POP_FIELD_NAME], graph.nodes[node][WHITE_POP], graph.nodes[node][BLACK_POP]
    d, r = graph.nodes[node][DEM_VOTE], graph.nodes[node][GOP_VOTE]
    nbrs = {nbr for nbr, datadict in graph.adj[node].items()}
    print(nbrs)
    nodeDict[node] = (white/totpop, black/totpop, d/(d+r))